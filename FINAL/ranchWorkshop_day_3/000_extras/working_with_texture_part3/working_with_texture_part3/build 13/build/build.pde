...
String[]      texNames    = { "0_1.png", "0_2.png", "0_3.png", "0_4.png" };
...
//                           0    1    2    3   4    5
int[]         texUV       = {50, 100, 200, 400, 800, 1200};
...
void setupART() {
	for (int i = 0; i < numAssets; ++i) {
		...
		d1.y = (int)random( 100, 600 );  // which scale to use ?
		d1.z = (int)random( 360 );       // which rotation to use ?
		data1[i] = d1;

		texZ[i] = new HOscillator().range(-2500, 250).speed(0.75).freq(1.0).currentStep( (int)random(333) );
	}
}
...
void setupUVXY() {
	for (int i = 0; i < numAssets; ++i) {
		...
		texOSC[i] = new HOscillator().range(0.0, 1.0).speed( 0.5 + ((int)random(4)*0.5) ).freq( 1.5 + ((int)random(4)*0.5) ).currentStep(i*5);
	}
}
